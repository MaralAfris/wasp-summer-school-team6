...
def start(graph, actions):
    ...
    rospy.Subscriber("/drone_goal_completed", PoseArray, obj.setDroneGoalStatus)

    # we need to block the rest of the program for some time in order
    # for the publishers and subcribers to innitiate themselves
    time.sleep(2) #
...
def moveTurtleBot(x,y,actionType, actionId, publisher):
    print "info: Moving", actionType, actionId
...
class Coordinator(object):
    ...
    # This is a function that is calledback by the topic
    def setDroneGoalStatus(self, data):
        print "Drone info: completed "
        action_type = data.poses[0].position.z
        actionId = data.poses[0].orientation.z
